...
export const process480p = async (key: string) => {
  const s3 = new S3({
    credentials: {
  ...
  try {
    ...
    fs.mkdirSync(`/tmp/480p/${key.split('.')[0]}`, {
      recursive: true,
    });
    ...
    const result = await new Promise(function (resolve, reject) {
      ffmpeg(readStream)
        .on('start', () => {
          console.log(`transcoding ${key.split('.')[0]} to 480p`);
        })
        ...
        .on('end', async () => {
          const fileUploadPromises = fs
            .readdirSync(`/tmp/480p/${key.split('.')[0]}`)
            .map((file) => {
              let params = {
                ...
                Body: fs.readFileSync(`/tmp/480p/${key.split('.')[0]}/${file}`),
              };
              console.log(`uploading ${file} to s3`);
              return s3.putObject(params).promise();
            });
          await Promise.all(fileUploadPromises);
          await fs.rmdirSync(`/tmp/480p/${key.split('.')[0]}`, {
            recursive: true,
          });
        ...
        .outputOptions([
          ...
          `-hls_segment_filename /tmp/480p/${key.split('.')[0]}/480p_%d.ts`,
        ])
        .output(`/tmp/480p/${key.split('.')[0]}/480p.m3u8`)
        .run();
    });
