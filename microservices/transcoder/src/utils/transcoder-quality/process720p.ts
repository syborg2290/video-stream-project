...
export const process720p = async (key: string, publicKey: string) => {
  const s3 = new S3({
    credentials: {
  ...
  try {
    ...
    fs.mkdirSync(`/tmp/720p/${key.split('.')[0]}`, {
      recursive: true,
    });
    ...
    const result = await new Promise(function (resolve, reject) {
      ffmpeg(readStream)
        .on('start', () => {
          console.log(`transcoding ${key.split('.')[0]} to 720p`);
        })
          ...
          const fileUploadPromises = fs
            ...
            .map((file) => {
              let params = {
                ...
                Body: fs.readFileSync(`/tmp/720p/${key.split('.')[0]}/${file}`),
              };
              console.log(`uploading ${file} to s3`);
              return s3.putObject(params).promise();
            });
          await Promise.all(fileUploadPromises);
          await fs.rmdirSync(`/tmp/720p/${key.split('.')[0]}`, {
            recursive: true,
          });
        ...
        .outputOptions([
          ...
          `-hls_segment_filename /tmp/720p/${key.split('.')[0]}/720p_%d.ts`,
        ])
        .output(`/tmp/720p/${key.split('.')[0]}/720p.m3u8`)
        .run();
    });
